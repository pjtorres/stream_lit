...
# Function to generate the graph (COMPLETELY FIXED community filtering)
def generate_graph(data, color_by_community, size_by_centrality, focus_community=None, graph_size="large"):
    ...
    # Apply community detection on FULL graph FIRST
    partition = None
    ...
    # CRITICAL: Apply community filtering AFTER partition is created
    ...
    if focus_community is not None and partition:
        ...
        # Get ALL nodes in the focused community
        focus_nodes = [node for node, comm in partition.items() if comm == focus_community]
        st.write(f"📍 Community {focus_community} contains {len(focus_nodes)} core nodes")
        
        if len(focus_nodes) == 0:
            st.error(f"❌ No nodes found in community {focus_community}!")
            st.write("Available communities:")
            for comm_id, nodes in {}:
                comm_nodes = [n for n, c in partition.items() if c == comm_id]
                if comm_nodes:
                    st.write(f"  Community {comm_id}: {len(comm_nodes)} nodes")
            return G, None, partition
        
        # Add direct neighbors from other communities (1-hop expansion)
        ...
        st.write(f"🔗 Added {neighbor_count} neighboring nodes from other communities")
        st.write(f"📊 **CREATING SUBGRAPH with {len(extended_nodes)} total nodes**")
        
        # Store original size for verification
        original_nodes = len(G.nodes())
        original_edges = len(G.edges())
        
        # Create the actual filtered subgraph
        if len(extended_nodes) > 0:
            G = G.subgraph(extended_nodes).copy()
            
            # Verify filtering worked
            new_nodes = len(G.nodes())
            new_edges = len(G.edges())
            
            st.write(f"✂️ **FILTERING RESULT:** {original_nodes}→{new_nodes} nodes, {original_edges}→{new_edges} edges")
            
            if new_nodes == original_nodes:
                st.error("❌ **FILTERING FAILED** - Graph size unchanged!")
            elif new_nodes < 10:
                st.warning(f"⚠️ Very small filtered graph: {new_nodes} nodes")
            else:
                st.success(f"✅ **FILTERING SUCCESS** - Reduced to {new_nodes} nodes")
            
            # Update partition to only include nodes in filtered graph
            partition = {node: original_partition[node] for node in G.nodes() if node in original_partition}
            
            # Verify partition update
            nodes_in_focus = len([n for n, c in partition.items() if c == focus_community])
            st.write(f"🎯 Focused community now has {nodes_in_focus} nodes in filtered view")
            
        else:
            st.error("❌ No nodes to include in subgraph!")
            return G, None, partition
    else:
    ...
    # Enhanced physics for better node separation
    net.set_options(f"""
    var options = {{
      "physics": {{
        "enabled": true,
    ...
    # Calculate centrality on the (possibly filtered) graph
    centrality_map = {
    ...
    # Add nodes with enhanced styling for focused community
    for node in G.nodes():
        # Determine node styling
        if color_by_community and partition and node in partition:
            node_color = community_colors[partition[node]]
            
            # HIGHLIGHT focused community nodes with WHITE BORDER and larger size
            if focus_community is not None and partition[node] == focus_community:
                border_color = "#FFFFFF"  # White border for focused nodes
                border_width = 6
                size_multiplier = 1.5
            else:
                border_color = "#666666"
        ...
        # Calculate node size
        if centrality:
            node_size = (centrality[node] * 80 + node_base_size) * size_multiplier
        else:
            node_size = node_base_size * size_multiplier
    ...
    # Add edges with enhanced styling for community focus
    for edge in G.edges(data=True):
        ...
        if focus_community is not None and partition:
            ...
            if node1_comm == focus_community and node2_comm == focus_community:
                # Internal edges in focused community - BRIGHT WHITE and THICK
                edge_color = "#FFFFFF"
                edge_width = 5
                edge_style = "solid"
            elif node1_comm == focus_community or node2_comm == focus_community:
                # Edges connecting to focused community - BRIGHT ORANGE
                edge_color = "#FF6600"
                edge_width = 3
                edge_style = "solid"
            else:
                # External edges - DIM GRAY and THIN
                edge_color = "#444444"
                edge_width = 1
                edge_style = "dashed"
        else:
