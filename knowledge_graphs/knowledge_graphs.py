...
# STREAMLIT WIDTH FIX - Set page config to wide layout
st.set_page_config(page_title="Interactive Knowledge Graphs", layout="wide")

# ADD CSS TO FORCE FULL WIDTH USAGE
st.markdown("""
<style>
...
# Function to generate the graph - ENHANCED FOR FULL WIDTH
def generate_graph(data, color_by_community, size_by_centrality):
    G = nx.Graph()
    for _, row in data.iterrows():
        G.add_edge(row['head'], row['tail'], label=row['relation'])

    # Create network with enhanced settings for full width display
    net = Network(
        height="900px", 
        width="100%", 
        notebook=False,
    ...
    # Add nodes to the graph with enhanced sizing
    for node in G.nodes():
        node_color = community_colors[partition[node]] if color_by_community else "#97c2fc"
        node_size = centrality[node] * 200 if centrality else 30  # Increased base size
        net.add_node(
            node,
            label=str(node),
    ...
    # Enhanced physics settings for better spread across full width
    net.repulsion(
        node_distance=150,     # Increased for better spread
        central_gravity=0.1,   # Reduced for more spread
        spring_length=250,     # Increased spring length
        spring_strength=0.005, # Reduced spring strength
        damping=0.95          # Added damping
    )
    
    # Add physics options for better interactivity
    net.set_options("""
    var options = {
      "physics": {
        "enabled": true,
...
# Function to create community subgraph visualization with 1-degree expansion - ENHANCED
def create_community_subgraph(G, partition, community_id, community_colors=None, expand_one_degree=False):
    ...
    # Create PyVis network with full width settings
    net = Network(
        height="700px", 
        width="100%", 
        notebook=False,
    ...
    # Enhanced physics for better spread
    net.repulsion(
        node_distance=120, 
        central_gravity=0.2, 
        spring_length=180, 
...
# FUNCTION TO FIX PYVIS HTML FOR FULL WIDTH
def fix_pyvis_html_width(html_content):
    """Inject CSS into PyVis HTML to force full width"""
    width_fix_css = """
    <style>
    body { 
        margin: 0 !important; 
    ...
    if all(col in data.columns for col in required_columns):
        ...
        if "graph" not in st.session_state or st.session_state.get("color_by_community") != color_by_community or st.session_state.get("size_by_centrality") != size_by_centrality:
            ...
            with open("knowledge_graph.html", 'r') as f:
                ...
                # APPLY WIDTH FIX TO THE HTML
                fixed_html = fix_pyvis_html_width(original_html)
                st.session_state["graph_html"] = fixed_html
        ...
        # Community Analysis Table (only when color_by_community is enabled)
        if color_by_community:
        ...
        # Display the graph - NOW WITH FULL WIDTH
        st.subheader("Knowledge Graph Visualization")
        
        # Use container to ensure full width and apply the fixed HTML
        with st.container():
            components.html(
        ...
        if user_question:
            ...
            # Show community that a specific node is part of
                ...
                if best_match and score > 70:
                    ...
                    with open("community_graph.html", 'r') as f:
                        ...
                        # APPLY WIDTH FIX TO COMMUNITY GRAPH TOO
                        fixed_community_html = fix_pyvis_html_width(community_html)
                    
                    st.write("**Community Visualization:**")
                    with st.container():
                        components.html(fixed_community_html, height=750, scrolling=False)
            ...
            # Show specific community by number
            ...
            # "who else is in the module" with 1-degree expansion
                ...
                if best_match and score > 70:
                    ...
                    if node_name in G.nodes():
                        ...
                        with open("module_graph.html", 'r') as f:
                            module_html = f.read()
                            # APPLY WIDTH FIX TO MODULE GRAPH TOO
                            fixed_module_html = fix_pyvis_html_width(module_html)
                        
                        expansion_text = " + 1-degree neighbors" if expand_module else ""
                        st.write(f"**Module {node_module} Visualization{expansion_text}:**")
                        with st.container():
                            components.html(fixed_module_html, height=750, scrolling=False)
                ...
                if " and " in cleaned_query:
                    ...
                    if node1 in G.nodes() and node2 in G.nodes():
                        try:
                            ...
                            # Highlight the shortest path in PyVis visualization - ENHANCED FOR FULL WIDTH
                            net = Network(
                                height="900px", 
                                width="100%", 
                                notebook=False,
                            ...
                            for node in G.nodes():
                                ...
                                node_size = 40 if node in shortest_path else 30
                                net.add_node(
                                    node, 
                                    label=node, 
                                    title=f"Node: {node}", 
                            ...
                            # Save and display the updated graph with width fix
                            net.save_graph("highlighted_graph.html")
                            with open("highlighted_graph.html", 'r') as f:
                                highlighted_html = f.read()
                                fixed_highlighted_html = fix_pyvis_html_width(highlighted_html)
                            
                            with st.container():
                                components.html(fixed_highlighted_html, height=950, scrolling=False)
